  ...
  try {
    ...
    if (!validationResult.success) {
      return NextResponse.json(
        { error: 'Please provide a valid moon name', code: ERROR_CODES.VALIDATION_ERROR },
        { status: 400 }
      );
    }
    ...
    if (!response.ok) {
      ...
      return NextResponse.json(
        { error: 'Failed to fetch moon data from NASA', code: ERROR_CODES.API_ERROR },
    ...
    return NextResponse.json(
      { error: 'No images found for this moon', code: ERROR_CODES.NOT_FOUND },
      { status: 404 }
    );
  } catch (error) {
    nasaLogger.error('[MOON_API]', error);
    return NextResponse.json(
      { error: 'Failed to fetch moon data', code: ERROR_CODES.UNKNOWN_ERROR },
      { status: 500 }
    );
  }
}
