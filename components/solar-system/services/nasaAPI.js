...
class NASAAPIService {
  ...
  async getMarsRoverPhoto(rover = 'curiosity', sol = null, earthDate = null) {
    ...
    if (sol !== null) params.append('sol', sol);
    if (earthDate !== null) params.append('earth_date', earthDate);

    const url = `/api/nasa/mars-rover?${params.toString()}`;
    const cacheKey = this._getCacheKey('mars_rover', { rover, sol, earthDate });

    try {
      return await this._fetchWithCache(url, cacheKey);
    } catch (err) {
      nasaLogger.warn('Mars rover data unavailable:', err.message);
      return null;
    }
  }
