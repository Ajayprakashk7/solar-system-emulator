// Planets.tsx - Enhanced with realistic astrophysics
'use client';
import { useMemo, useEffect, useRef } from "react";
import { TextureLoader, Mesh, Group, Texture, MeshPhysicalMaterial } from "three";
...
export default function Planet({
  ...
  const { updatePlanetPosition } = usePlanetPositions() as PlanetPositionsContextType;
  ...
  const { overrideSpeedFactor, speedFactor } = useSpeedControl() as SpeedControlContextType;
  ...
  const planetData = useMemo(() => (planetsData as unknown as PlanetData[]).find(p => p.id === id), [id]);
  ...
  const handlePlanetClick = () => {
    const planetData = (planetsData as unknown as PlanetData[]).find(planet => planet.id === id);
    if (planetData) {
  ...
  return (
    <>
      <group ref={groupRef} position={[orbitRadius, 0, 0]} rotation={[tilt, 0, 0]}>
        ...
        {moons && moons.length > 0 && planetData && (
          <Moons
