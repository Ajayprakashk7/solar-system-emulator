...
function Planet({
  ...
  // Atmosphere only needs minimal segments
  const atmosphereSphereArgs = useMemo(() => [radius * 1.03, Math.max(16, Math.floor(settings.sphereSegments / 2)), Math.max(16, Math.floor(settings.sphereSegments / 2))], [radius, settings.sphereSegments]);
