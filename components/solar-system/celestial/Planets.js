...
function Planet({
  ...
  // Detailed LOD geometries
  const lodArgsHigh = useMemo(() => [radius, 32, 32], [radius]);
  const lodArgsMid = useMemo(() => [radius, 16, 16], [radius]);
  const lodArgsLow = useMemo(() => [radius, 8, 8], [radius]);
        ...
        <group ref={ref} onClick={handlePlanetClick}>
          <Detailed distances={[0, 15, 30]}>
            <mesh castShadow receiveShadow>
              <sphereGeometry args={lodArgsHigh} />
              <meshStandardMaterial {...materialProps} />
            </mesh>
            <mesh castShadow receiveShadow>
              <sphereGeometry args={lodArgsMid} />
              <meshStandardMaterial {...materialProps} />
            </mesh>
            <mesh castShadow receiveShadow>
              <sphereGeometry args={lodArgsLow} />
              <meshStandardMaterial {...materialProps} />
            </mesh>
          </Detailed>
        </group>
