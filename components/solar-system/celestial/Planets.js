...
function Planet({
  ...
  // LOD geometry arguments
  const highSphereArgs = useMemo(() => [radius, 32, 32], [radius]);
  const medSphereArgs = useMemo(() => [radius, 16, 16], [radius]);
  const lowSphereArgs = useMemo(() => [radius, 8, 8], [radius]);
  ...
  return (
    <>
      <group ref={groupRef} position={[orbitRadius, 0, 0]} rotation={[tilt, 0, 0]}>
        {/* Main planet using LOD to reduce vertices at distance */}
        <Detailed distances={[0, 15, 30]} ref={ref} onClick={handlePlanetClick}>
          {/* High detail (close up) */}
          <mesh castShadow receiveShadow>
            <sphereGeometry args={highSphereArgs} />
            <meshStandardMaterial {...materialProps} />
          </mesh>

          {/* Medium detail */}
          <mesh castShadow receiveShadow>
            <sphereGeometry args={medSphereArgs} />
            <meshStandardMaterial {...materialProps} />
          </mesh>

          {/* Low detail (far away) */}
          <mesh castShadow receiveShadow>
            <sphereGeometry args={lowSphereArgs} />
            <meshStandardMaterial {...materialProps} />
          </mesh>
        </Detailed>
