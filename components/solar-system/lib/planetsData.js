// Sun has radius of 1 and position of 0,0,0
import { Vector3 } from 'three';

const baseOrbitSpeed = 1;

const planetsData = [
  {
    id: 0,
    name: "Sun",
    texturePath: "/images/bodies/sun_2k.webp",
    position: new Vector3(0, 0, 0),
    radius: 1,
    rotationSpeed: 0.5,
    tilt: 0,
    orbitSpeed: 0, // Sun doesn't orbit
    displayStats: {
      classification: "G-type main-sequence star",
      orbitalPeriod: 0,
      meanDistanceFromSun: 0,
      accurateRadius: 696340,
      mass: 333000, // Earth masses
      surfaceGravity: 28.02,
      rotationPeriod: 25.05, // At equator
      axialTilt: 7.25,
      numberOfMoons: 0,
      atmosphericComposition: "Hydrogen (~73%), Helium (~25%), Other elements (~2%)",
      surfaceTemp: "5778 K (5505°C)",
      coreTemp: "15 million°C",
      age: "4.6 billion years",
      type: "Yellow dwarf star"
    },
    moons: [],
    isSun: true, // Special flag to identify the Sun
  },
  {
    id: 1,
    name: "Mercury",
    texturePath: "/images/bodies/mercury_2k.webp",
    position: new Vector3(1.5, 0, 0),
    radius: 0.1,
    rotationSpeed: 1,
    tilt: 0.00017,
    orbitSpeed: baseOrbitSpeed / 0.24,
    displayStats: {
      classification: "Terrestrial planet",
      orbitalPeriod: 88,
      meanDistanceFromSun: 0.39,
      accurateRadius: 2440,
      mass: 0.055,
      surfaceGravity: 0.38,
      rotationPeriod: 58.6,
      axialTilt: 0.034,
      numberOfMoons: 0,
      atmosphericComposition: "Trace amounts of hydrogen, helium, oxygen, sodium, calcium, potassium",
      surfaceTemp: "-173 to 427°C",
    },
    moons: [],
  },
  {
    id: 2,
    name: "Venus",
    texturePath: "/images/bodies/venus_surface_2k.webp",
    position: new Vector3(2.2, 0, 0),
    radius: 0.15,
    rotationSpeed: 1,
    tilt: 3.09639,
    orbitSpeed: baseOrbitSpeed / 0.62,
    displayStats: {
      classification: "Terrestrial planet",
      orbitalPeriod: 225,
      meanDistanceFromSun: 0.72,
      accurateRadius: 6052,
      mass: 0.815,
      surfaceGravity: 0.904,
      rotationPeriod: -243,
      axialTilt: 177.4,
      numberOfMoons: 0,
      atmosphericComposition: "Carbon dioxide, nitrogen",
      surfaceTemp: "462°C",
    },
    moons: [],
  },
  {
    id: 3,
    name: "Earth",
    texturePath: "/images/bodies/earth_2k.webp",
    position: new Vector3(3, 0, 0),
    radius: 0.15,
    rotationSpeed: 1,
    tilt: 0.40928,
    orbitSpeed: 0.6,
    displayStats: {
      classification: "Terrestrial planet",
      orbitalPeriod: 365.25,
      meanDistanceFromSun: 1,
      accurateRadius: 6371,
      mass: 1,
      surfaceGravity: 1,
      rotationPeriod: 23.93,
      axialTilt: 23.44,
      numberOfMoons: 1,
      atmosphericComposition: "Nitrogen, oxygen",
      surfaceTemp: "-88 to 58°C",
    },
    moons: [
      {
        name: "Moon",
        radius: 0.041,
        orbitRadius: 0.6,
        orbitSpeed: 0.5,
        color: "#888888",
        surfaceDetail: "heavily cratered",
      }
    ],
  },
  {
    id: 4,
    name: "Mars",
    texturePath: "/images/bodies/mars_2k.webp",
    position: new Vector3(4, 0, 0),
    radius: 0.13,
    rotationSpeed: 0.5,
    tilt: 0.43965,
    orbitSpeed: baseOrbitSpeed / 1.88,
    displayStats: {
      classification: "Terrestrial planet",
      orbitalPeriod: 687,
      meanDistanceFromSun: 1.52,
      accurateRadius: 3390,
      mass: 0.107,
      surfaceGravity: 0.376,
      rotationPeriod: 24.6,
      axialTilt: 25.19,
      numberOfMoons: 2,
      atmosphericComposition: "Carbon dioxide, nitrogen, argon",
      surfaceTemp: "-125 to 20°C",
    },
    moons: [
      {
        name: "Phobos",
        radius: 0.02,
        orbitRadius: 0.35,
        orbitSpeed: 1.5,
        color: "#4a4a4a",
        surfaceDetail: "irregular, cratered",
      },
      {
        name: "Deimos",
        radius: 0.015,
        orbitRadius: 0.55,
        orbitSpeed: 0.8,
        color: "#555555",
        surfaceDetail: "smooth, less cratered",
      }
    ],
  },
  {
    id: 5,
    name: "Jupiter",
    texturePath: "/images/bodies/jupiter_2k.webp",
    position: new Vector3(6, 0, 0),
    radius: 0.25,
    rotationSpeed: 0.2,
    tilt: 0.05463,
    orbitSpeed: baseOrbitSpeed / 11.86,
    displayStats: {
      classification: "Gas giant",
      orbitalPeriod: 11.86,
      meanDistanceFromSun: 5.20,
      accurateRadius: 69911,
      mass: 317.8,
      surfaceGravity: 2.528,
      rotationPeriod: 9.93,
      axialTilt: 3.13,
      numberOfMoons: 79,
      atmosphericComposition: "Hydrogen, helium",
      surfaceTemp: "-145°C",
    },
    moons: [
      {
        name: "Io",
        radius: 0.055,
        orbitRadius: 0.7,
        orbitSpeed: 1.2,
        color: "#ffcc44",
        surfaceDetail: "volcanic, sulfurous",
      },
      {
        name: "Europa",
        radius: 0.047,
        orbitRadius: 1.0,
        orbitSpeed: 0.9,
        color: "#aaddff",
        surfaceDetail: "icy, smooth cracks",
      },
      {
        name: "Ganymede",
        radius: 0.08,
        orbitRadius: 1.4,
        orbitSpeed: 0.6,
        color: "#8899aa",
        surfaceDetail: "mixed ice and rock",
      },
      {
        name: "Callisto",
        radius: 0.073,
        orbitRadius: 1.8,
        orbitSpeed: 0.4,
        color: "#665544",
        surfaceDetail: "ancient, heavily cratered",
      }
    ],
  },
  {
    id: 6,
    name: "Saturn",
    texturePath: "/images/bodies/saturn_2k.webp",
    position: new Vector3(9, 0, 0),
    radius: 0.2,
    rotationSpeed: 0.1,
    tilt: 0.46653,
    orbitSpeed: baseOrbitSpeed / 29.46,
    rings: {
      texturePath: "/images/bodies/saturn_ring.webp",
      size: [0.2, 0.46],
    },
    displayStats: {
      classification: "Gas giant",
      orbitalPeriod: 29.45,
      meanDistanceFromSun: 9.58,
      accurateRadius: 58232,
      mass: 95.2,
      surfaceGravity: 1.065,
      rotationPeriod: 10.7,
      axialTilt: 26.73,
      numberOfMoons: 82,
      atmosphericComposition: "Hydrogen, helium",
      surfaceTemp: "-178°C",
    },
    moons: [
      {
        name: "Mimas",
        radius: 0.03,
        orbitRadius: 0.55,
        orbitSpeed: 1.1,
        color: "#cccccc",
        surfaceDetail: "Death Star crater",
      },
      {
        name: "Enceladus",
        radius: 0.038,
        orbitRadius: 0.75,
        orbitSpeed: 0.95,
        color: "#f0f8ff",
        surfaceDetail: "icy geysers",
      },
      {
        name: "Tethys",
        radius: 0.042,
        orbitRadius: 0.9,
        orbitSpeed: 0.85,
        color: "#ddeeff",
        surfaceDetail: "icy, smooth",
      },
      {
        name: "Dione",
        radius: 0.045,
        orbitRadius: 1.1,
        orbitSpeed: 0.75,
        color: "#d0d8e0",
        surfaceDetail: "wispy terrain",
      },
      {
        name: "Rhea",
        radius: 0.058,
        orbitRadius: 1.4,
        orbitSpeed: 0.6,
        color: "#c8d0d8",
        surfaceDetail: "heavily cratered",
      },
      {
        name: "Titan",
        radius: 0.078,
        orbitRadius: 2.0,
        orbitSpeed: 0.4,
        color: "#ffcc88",
        surfaceDetail: "thick atmosphere, lakes",
      },
      {
        name: "Iapetus",
        radius: 0.056,
        orbitRadius: 2.6,
        orbitSpeed: 0.25,
        color: "#888888",
        surfaceDetail: "two-toned coloring",
      }
    ],
  },
  {
    id: 7,
    name: "Uranus",
    texturePath: "/images/bodies/uranus_2k.webp",
    position: new Vector3(13, 0, 0),
    radius: 0.18,
    rotationSpeed: 0.07,
    tilt: 1.70622,
    orbitSpeed: baseOrbitSpeed / 84.01,
    displayStats: {
      classification: "Ice giant",
      orbitalPeriod: 84,
      meanDistanceFromSun: 11.22,
      accurateRadius: 25362,
      mass: 14.5,
      surfaceGravity: 0.886,
      rotationPeriod: -17.2,
      axialTilt: 97.77,
      numberOfMoons: 27,
      atmosphericComposition: "Hydrogen, helium, methane",
      surfaceTemp: "-224°C",
    },
    moons: [
      {
        name: "Miranda",
        radius: 0.036,
        orbitRadius: 0.6,
        orbitSpeed: 1.3,
        color: "#bfbfbf",
        surfaceDetail: "extreme terrain",
      },
      {
        name: "Ariel",
        radius: 0.044,
        orbitRadius: 0.9,
        orbitSpeed: 1.0,
        color: "#c8c8c8",
        surfaceDetail: "valleys, fault canyons",
      },
      {
        name: "Umbriel",
        radius: 0.045,
        orbitRadius: 1.2,
        orbitSpeed: 0.8,
        color: "#5a5a5a",
        surfaceDetail: "dark surface",
      },
      {
        name: "Titania",
        radius: 0.06,
        orbitRadius: 1.6,
        orbitSpeed: 0.6,
        color: "#a8a8a8",
        surfaceDetail: "icy, cratered",
      },
      {
        name: "Oberon",
        radius: 0.058,
        orbitRadius: 2.0,
        orbitSpeed: 0.45,
        color: "#999999",
        surfaceDetail: "heavily cratered, icy",
      }
    ],
  },
  {
    id: 8,
    name: "Neptune",
    texturePath: "/images/bodies/neptune_2k.webp",
    position: new Vector3(17, 0, 0),
    radius: 0.18,
    rotationSpeed: 0.06,
    tilt: 0.49428,
    orbitSpeed: baseOrbitSpeed / 164.8,
    displayStats: {
      classification: "Ice giant",
      orbitalPeriod: 164.8,
      meanDistanceFromSun: 30.07,
      accurateRadius: 24622,
      mass: 17.15,
      surfaceGravity: 1.14,
      rotationPeriod: 16.11,
      axialTilt: 28.32,
      numberOfMoons: 14,
      atmosphericComposition: "Hydrogen, helium, methane",
      surfaceTemp: "-214°C",
    },
    moons: [
      {
        name: "Triton",
        radius: 0.065,
        orbitRadius: 1.2,
        orbitSpeed: -1.1,
        color: "#ffddee",
        surfaceDetail: "nitrogen geysers, retrograde",
      },
      {
        name: "Proteus",
        radius: 0.032,
        orbitRadius: 0.7,
        orbitSpeed: 1.3,
        color: "#a0a0a0",
        surfaceDetail: "irregular, dark",
      }
    ],
  },

  {
    id: 9,
    name: 'Pluto',
    texturePath: '/images/bodies/moon_2k.webp', // fallback texture
    position: new Vector3(20, 0, 0),
    radius: 0.08,
    rotationSpeed: 0.1,
    tilt: 2.138,
    orbitSpeed: baseOrbitSpeed / 248.0,
    displayStats: {
      classification: 'Dwarf planet',
      orbitalPeriod: 90560, // days
      meanDistanceFromSun: 39.48, // AU
      accurateRadius: 1188.3, // km
      mass: 0.00218, // Earth masses
      surfaceGravity: 0.063, // g
      rotationPeriod: -153.3, // hours
      axialTilt: 122.53, // degrees
      numberOfMoons: 5,
      atmosphericComposition: 'Nitrogen, methane, carbon monoxide',
      surfaceTemp: '-229°C',
    },
    moons: [
      {
        name: 'Charon',
        radius: 0.04,
        orbitRadius: 0.6,
        orbitSpeed: 0.8,
        color: '#aaaaaa',
        surfaceDetail: 'large relative to Pluto, icy',
      }
    ],
  },
];

export default planetsData;
