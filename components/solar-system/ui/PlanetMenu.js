        ...
        <div className="flex gap-2 overflow-x-auto overflow-y-hidden px-1 pb-2 snap-x snap-mandatory scroll-smooth scrollbar-hide max-w-full">
          {planets.map((planet) => (
            <button
              ...
              className={`px-3 py-2 rounded-lg text-xs font-medium min-h-[36px] min-w-[64px] flex-shrink-0 snap-start transition-all duration-200 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-purple-500 focus-visible:ring-offset-2 focus-visible:ring-offset-black ${
      ...
      <div className="hidden sm:flex flex-wrap justify-center gap-3" role="navigation" aria-label="Planet selection menu">
        {planets.map((planet) => (
          <button
            ...
            aria-label={`View ${planet.name} details`}
            aria-pressed={selectedPlanet?.id === planet.id}
            className={`px-4 py-2 rounded-lg text-sm font-medium min-w-[80px] min-h-[40px] transition-all duration-200 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-purple-500 focus-visible:ring-offset-2 focus-visible:ring-offset-black ${
              selectedPlanet?.id === planet.id
                ? 'bg-purple-600 text-white opacity-50 cursor-not-allowed'
                : 'bg-purple-500/20 text-purple-200 hover:bg-purple-500/40 hover:text-white'
            }`}
