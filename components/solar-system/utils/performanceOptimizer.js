...
export class FPSMonitor {
  ...
  tick(now) {
    if (this.lastTime > 0) {
      ...
      const dropped = this.frameCount >= this.sampleSize ? this.frameTimes[this.frameIndex] : 0;
      this.frameTimes[this.frameIndex] = delta;
      this.frameTimesSum = this.frameTimesSum + delta - dropped;
    ...
    if (this.frameCount >= this.sampleSize) {
      const avgDelta = this.frameTimesSum / this.sampleSize;
