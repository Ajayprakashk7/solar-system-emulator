...
export class FPSMonitor {
  constructor({ targetFPS = 55, sampleSize = 60, onDowngrade, onUpgrade } = {}) {
    ...
    // Optimization: Use pre-allocated Float64Array and running sum for O(1) sliding window average
    // to eliminate garbage collection pressure and O(N) array operations in high-frequency loops.
  ...
  tick(now) {
    if (this.lastTime > 0) {
      ...
      // Remove oldest value from sum, add new value
      this.frameSum -= this.frameTimes[this.frameIndex];
    ...
    if (this.frameCount >= this.sampleSize) {
      const avgDelta = this.frameSum / this.sampleSize;
  ...
  getAverageFPS() {
    ...
    const avgDelta = this.frameSum / this.frameCount;
    return 1 / avgDelta;
  }
}
