...
export class FPSMonitor {
  constructor({ targetFPS = 55, sampleSize = 60, onDowngrade, onUpgrade } = {}) {
    ...
    // ⚡ Bolt: Replace dynamic array with a fixed-size circular buffer for O(1) time complexity
  ...
  /**
   * Resets the circular buffer and all associated counters
   */
  reset() {
  ...
  tick(now) {
    if (this.lastTime > 0) {
      ...
      // ⚡ Bolt: Maintain a running sum by subtracting the oldest value and adding the newest
      const oldestDelta = this.frameCount === this.sampleSize ? this.frameTimes[this.frameIndex] : 0;
      this.frameTimes[this.frameIndex] = delta;
      this.frameTimesSum += delta - oldestDelta;
    ...
    if (this.frameCount >= this.sampleSize) {
      const avgDelta = this.frameTimesSum / this.frameCount;
