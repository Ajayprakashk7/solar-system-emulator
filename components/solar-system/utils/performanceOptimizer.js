...
export class FPSMonitor {
  constructor({ targetFPS = 55, sampleSize = 60, onDowngrade, onUpgrade } = {}) {
    ...
    // O(1) circular buffer optimizations for high frequency tick updates
  ...
  tick(now) {
    if (this.lastTime > 0) {
      ...
      // Update circular buffer and running sum in O(1)
      if (this.frameCount === this.sampleSize) {
        this.frameTimesSum -= this.frameTimes[this.frameIndex];
      } else {
        this.frameCount++;
      }
      this.frameTimes[this.frameIndex] = delta;
      this.frameTimesSum += delta;
    ...
    if (this.frameCount >= this.sampleSize) {
      ...
      if (avgFPS < this.targetFPS * 0.7 && this.onDowngrade) {
        ...
        this.frameCount = 0; // Reset buffer
        this.frameTimesSum = 0;
