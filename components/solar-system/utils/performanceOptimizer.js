...
export class FPSMonitor {
  ...
  tick(now) {
    if (this.lastTime > 0) {
      ...
      // Update running sum by removing the oldest value and adding the new delta
      this.runningSum = this.runningSum - this.frameTimes[this.currentIndex] + delta;

      // Add new delta to buffer
      this.frameTimes[this.currentIndex] = delta;

      // Update index and count
      this.currentIndex = (this.currentIndex + 1) % this.sampleSize;
  ...
  getAverageFPS() {
    ...
    const avgDelta = this.runningSum / this.count;
    return 1 / avgDelta;
  }
}
